...
Import `Share2` alongside the existing `X` from `lucide-react`. In the keydown
handler, make the WHOLE handler conditional — the guard goes at the very top,
above the Escape branch as well as the Tab handling:

```tsx
      if (!trapActive) return
```

The guard must cover Escape too. Both modals register a `window` keydown
listener, so one Escape press reaches both: without the guard, dismissing the
share panel also closes the letter underneath it, and a reader who only meant
to put the share panel away is thrown out of the letter entirely. While the
share panel owns the trap, the letter ignores Escape; when the panel closes,
the letter's own Escape resumes.
